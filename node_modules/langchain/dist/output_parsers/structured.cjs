...
function printSchema(schema, depth = 0) {
    if ((schema instanceof zod_1.z.ZodString || schema._def.typeName === "ZodString") &&
    ...
    if (schema instanceof zod_1.z.ZodNumber || schema._def.typeName === "ZodNumber") {
        return "number";
    }
    if (schema instanceof zod_1.z.ZodBoolean || schema._def.typeName === "ZodBoolean") {
        return "boolean";
    }
    if (schema instanceof zod_1.z.ZodDate || schema._def.typeName === "ZodDate") {
        return "date";
    }
    ...
    if (schema instanceof zod_1.z.ZodTransformer ||
    ...
    if (schema instanceof zod_1.z.ZodArray || schema._def.typeName === "ZodArray") {
        return `${printSchema(schema._def.type, depth)}[]`;
    }
    if (schema instanceof zod_1.z.ZodObject || schema._def.typeName === "ZodObject") {
